...
OUT = os.path.join(HERE, '..', 'inside', 'rag_run_guide_rev1.xlsx')
...
ws['A2'] = '2026-09-25 · 밖(사외) 6개 → 사내 전체 · 연노랑 = 기입 · 밖에서는 설정·키 불필요'
ws['A2'].font = f(size=9, color='6B6B6B')
intro = [
 ('왜 14개인가', '한 노트북 = 한 가지 질문. 섞어 놓으면 어느 단계가 문제인지 구분되지 않아 나눠 둠. 전부 돌릴 필요는 없음'),
 ('밖에서 6개', 'nb01 · nb02 · nb00 · nb03 · nb06(검색 지표) · nb10(답변 지표) — LLM 없이 전부 동작'),
 ('LLM 필요 2개', 'nb12 골든셋 생성 · nb13 프리셋 비교 — 사내에서'),
 ('나머지 6개', 'nb04 Milvus · nb05 스모크 · nb07 정형연계 · nb08 자동평가 · nb09 · nb11 — 필요할 때'),
 ('밖에서 준비', '설정 없음. 압축 풀고 pandas · numpy · openpyxl 만 설치하면 문서 100건 · 정답지 30문항으로 바로 실행'),
...
sheets = [
 ('1_사외연습_체크리스트', '★ 밖에서 하나씩 돌리며 확인 체크 — 무엇을 보고 무엇을 확인했는지'),
 ('1b_노트북_연습순서', '연습 순서 요약 (소요 시간 · LLM 필요 여부)'),
...
# ================= 1. 사외 연습 체크리스트 =================
ws = wb.create_sheet('1_사외연습_체크리스트')
