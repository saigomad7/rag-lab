...
| 파일 | 무엇 |
|---|---|
| `rag_checklist_sheet_rev5.html` | **★ 진행 기준** — 스프레드시트 형식 체크리스트. 요약 · 소스별 매트릭스 · 소스별 상세 · **소스별 컬럼 정의** · 소스별 처리 흐름 · P1 적재/검색 · P2 센싱 · P3 정형연계 · 정형연계 흐름 · 스모크 20 |
| `rag_checklist_rev5.xlsx` | 위 체크리스트의 **엑셀판** — 상태 드롭다운, 요약 자동 계산(수식). 사내에서 채워 가며 쓰는 용도 |
| `rag_checklist_rev5.xlsx` 의 `소스별_컬럼정의` 시트 | **소스별 컬럼 정의서** — 소스마다 있어야 할 컬럼(권장안) 91행. J·K 열(사내 보유 · 사내 컬럼명)을 채우면 사내 현황 매핑표가 됨 |
