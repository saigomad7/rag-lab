...
| `rag_checklist_rev9.xlsx` | 위 체크리스트의 **엑셀판** — 상태 드롭다운, 요약 자동 계산(수식). 사내에서 채워 가며 쓰는 용도 |
| `rag_checklist_rev9.xlsx` 의 `소스별_컬럼정의` · `JSON_스키마` 시트 | **소스별 컬럼 정의서** — 필요한 항목 98개(공통 20 · 본문/요약 14 · 청크 12 · 소스별 52 · 소스 7종)와 **저장 위치**(공통 컬럼 / META_EXTRA JSON / TAGS / ACL 행). K·L 열(사내 보유 · 사내 컬럼명)을 채우면 사내 테이블 매핑표. JSON_스키마 시트에 실제 JSON 모양과 매핑 4가지 경우 |
...
| `golden_eval_pack_rev3.xlsx` | **★ 최신 · 사내 기입용** — rev2 + **`2d_근거문서_연결`**(질문↔코퍼스 근거 확정 · 근거 확보율) · **`2e_원본_수집목록`**(원본 URL·수집·파싱·청킹·인덱싱 상태 대장) · 안내에 측정 성립 조건 · 실패 3분류 추가 |
