...
| 파일 | 무엇 |
|---|---|
| `rag_checklist_sheet_rev8.html` | **★ 진행 기준** — 스프레드시트 형식 체크리스트. 요약 · 소스별 매트릭스 · 소스별 상세 · **소스별 컬럼 정의** · 소스별 처리 흐름 · P1 적재/검색 · P2 센싱 · P3 정형연계 · 정형연계 흐름 · 스모크 20 |
| `rag_checklist_rev8.xlsx` | 위 체크리스트의 **엑셀판** — 상태 드롭다운, 요약 자동 계산(수식). 사내에서 채워 가며 쓰는 용도 |
| `rag_checklist_rev8.xlsx` 의 `소스별_컬럼정의` · `JSON_스키마` 시트 | **소스별 컬럼 정의서** — 필요한 항목 112개(공통 20 · **본문/요약 14** · 청크 12 · 소스별 66)와 **저장 위치**(공통 컬럼 / META_EXTRA JSON / TAGS / ACL 행). K·L 열(사내 보유 · 사내 컬럼명)을 채우면 사내 테이블 매핑표. JSON_스키마 시트에 실제 JSON 모양과 매핑 4가지 경우 |
