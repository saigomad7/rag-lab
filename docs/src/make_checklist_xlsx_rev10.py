...
import os as _os
OUT = _os.path.join(_os.path.dirname(_os.path.abspath(__file__)), '..', 'rag_checklist_rev10.xlsx')
FONT = '맑은 고딕'
...
# ---------------- 단계별 실행 도구 (S번호 ↔ 노트북 ↔ 지표 ↔ 기록 시트) ----------------
TOOLMAP = [
 ('S00', '적재 검증', 'nb00_pipeline_check', '미달 항목 0 건 · doc_id 유일성 · 메타 충족률 · 고아 청크 · 인덱싱 일치', '이력대장 3_파이프라인점검'),
 ('S01', '수집', 'nb00 · nb01_inventory', '소스별 문서 수 · 기간 분포 · 날짜 누락률 · 본문 중복률', '이력대장 3_파이프라인점검'),
 ('S02', '파싱', 'nb02_parse_chunk', '노이즈 잔존율 · 표 손상 · 문장 중간 절단률', '이력대장 3_파이프라인점검'),
 ('S03', '청킹', 'nb02 · nb00', '청크 길이 분포 · 초단문 비율 · 순번 결손 · Context Recall', '이력대장 3_파이프라인점검'),
 ('S04', '임베딩', 'nb00 [6] · nb04_milvus', '토큰 p95 · EMBED_MAX_LENGTH 초과율', '이력대장 3_파이프라인점검'),
 ('S05', '메타데이터', 'nb00 [4] · nb01', '소스별 필수 메타 충족률 (기준 0.95)', '체크리스트 소스별_컬럼정의'),
 ('S06', '벡터 DB', 'nb04_milvus · nb00 [7]', '인덱싱 건수 / 청크 건수 · 필드 구성 · 자기검색 적중률', '이력대장 3_파이프라인점검'),
 ('S07', '질의 입력', 'nb05_smoke_test', '스모크 20문항 판정 · 실패 유형 5종 집계', '평가팩 스모크 · 체크리스트 스모크20'),
 ('S08', '질의 분석 · 분해', 'nb13 (decompose 노드)', '분해 적용 시 Recall 변화 · 하위 질의 수', '평가팩 3_평가_실행기록'),
 ('S09', 'BM25', 'nb03_bm25_tokenizer', '토크나이저별(공백 · 조사 · Kiwi) 적중률', '평가팩 3_평가_실행기록'),
 ('S10', '시맨틱 검색', 'nb04 · nb06_retrieval_eval', 'dense · sparse 단독 Recall@k', '평가팩 3_평가_실행기록'),
 ('S11', '결합 · 다양성', 'nb06 · nb13 (v1 · v3)', 'RRF 결합 · MMR 적용 전후 Precision · Context Precision', '이력대장 6_프리셋비교'),
 ('S12', '리랭크', 'nb06 · nb13 (v2)', 'MRR · nDCG@10 개선 폭 · 지연 증가', '이력대장 6_프리셋비교'),
 ('S13', '컨텍스트', 'nb10 · nb13 (grade 노드)', 'Context Recall · Context Precision', '평가팩 7_답변평가'),
 ('S14', 'LLM 답변', 'nb10_answer_eval', 'Faithfulness · Answer Correctness · Citation · Refusal', '평가팩 7_답변평가 · 8_표준지표'),
 ('S15', '평가', 'nb08 · nb09 · nb11 · nb12', '골든셋 구성 · 근거 확보율 · 회차별 지표', '평가팩 전체 · 이력대장 7_지표현황'),
 ('S16', '센싱 축', '— (Phase 2 · 미착수)', '축별 신호 정의', '이력대장 9_다음할일'),
 ('S17', '상시 질의', '— (Phase 2 · 미착수)', '축당 질의 2~4개', '이력대장 9_다음할일'),
 ('S18', '검색 · 수집', '— (Phase 2 · 미착수)', '기간 창 비교', '이력대장 9_다음할일'),
 ('S19', '변화 감지', '— (Phase 2 · 미착수)', '이상 점수 · 임계', '이력대장 9_다음할일'),
 ('S20', '요약 · 알림', '— (Phase 2 · 미착수)', '알림 정확도', '이력대장 9_다음할일'),
 ('S21', '백테스트', '— (Phase 2 · 미착수)', '과거 사건 재현율', '이력대장 9_다음할일'),
 ('S22', '정형 자산 카탈로그', 'nb07_sql_router [1]', '지표 정의서 채움률', '지표정의서 양식 xlsx'),
 ('S23', '시맨틱 뷰 · 허용목록', 'nb07 [2]', 'allow_list 적용 여부', '지표정의서 allow_list'),
 ('S24', '질의 라우팅', 'nb07 [3] · lab_graph route', '라우팅 정확도 (sql · rag · hybrid)', '평가팩 3_평가_실행기록'),
 ('S25', 'SQL 생성 · 검증', 'nb07 [4~6]', '정적 검증 차단률 100% · 실행 정확도(EX)', '이력대장 7_지표현황'),
 ('S26', '정형 × 비정형 결합', 'nb07 [7]', '결합 답변 수치 일치율', '평가팩 7_답변평가'),
 ('S27', '수치 정합성 · 신뢰', 'nb07 [8]', 'DB 우선 원칙 준수 · 시점 병기', '평가팩 7_답변평가'),
 ('S28', '권한 · 보안', 'nb07 · lab_sql.validate', '차단 규칙(SELECT 전용 · 원본 테이블 · 다중문) 통과', '이력대장 8_이슈결정'),
 ('S29', '평가 · 운영', 'nb07 [9] · nb13', 'EX · p95 지연 · 재현성', '이력대장 7_지표현황'),
]
wt = wb.create_sheet('단계별_실행도구')
wt['A1'] = '단계별 실행 도구 — 체크리스트 단계(S번호) ↔ 노트북 ↔ 확인 지표'
wt['A1'].font = f(size=13, bold=True, color=PRI)
wt['A2'] = '각 단계를 무엇으로 확인하고 결과를 어디에 기록하는지 대응표 · 실행 순서는 docs/rag_runbook_rev1.html'
wt['A2'].font = f(size=9, color='6B6B6B')
wt.merge_cells('A1:F1'); wt.merge_cells('A2:F2')
for j, c in enumerate(['단계', '이름', '실행 도구(노트북)', '확인 지표 · 합격 기준', '결과 기록 위치', '상태'], 1):
    x = wt.cell(4, j, c); x.font = f(bold=True); x.fill = HDR_FILL; x.border = BOX
    x.alignment = Alignment(horizontal='center', vertical='center', wrap_text=True)
for i, row in enumerate(TOOLMAP, 5):
    for j, v in enumerate(list(row) + [''], 1):
        x = wt.cell(i, j, v); x.border = BOX
        x.font = f(bold=(j == 1), color=('808080' if '미착수' in str(row[2]) else '1F1F1F'))
        x.alignment = Alignment(wrap_text=True, vertical='top',
                                horizontal=('center' if j in (1, 6) else 'left'))
        if j == 6:
            x.fill = INPUT_FILL
dvt = DataValidation(type='list', formula1='"완료,진행,미착수,해당없음"', allow_blank=True)
wt.add_data_validation(dvt); dvt.add(f'F5:F{4 + len(TOOLMAP)}')
widths(wt, [7, 20, 26, 54, 30, 10])
wt.freeze_panes = 'A5'
wt.auto_filter.ref = f'A4:F{4 + len(TOOLMAP)}'
wt.page_setup.orientation = 'landscape'; wt.sheet_properties.pageSetUpPr.fitToPage = True
wt.page_setup.fitToWidth = 1; wt.page_setup.fitToHeight = 0

# 시트 순서 정리
order = ['안내', '요약', '단계별_실행도구', '소스별_매트릭스', '소스별_상세', '소스별_컬럼정의', 'JSON_스키마', '처리흐름_도식', '처리흐름_판단기준', 'P1_적재공통', 'P1_검색', 'P2_센싱', 'P3_정형연계', '정형연계_패턴', '스모크20']
