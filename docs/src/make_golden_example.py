# -*- coding: utf-8 -*-
"""골든셋 실물 예시 → golden_set_example_rev1.xlsx
   샘플 데이터로 실제 생성한 값이 들어간다 (수동 골든셋 · 자동 평가셋 4종 · 정형 골든셋 · 스모크 20)."""
import os
import sys
import pandas as pd
...
import lab_io, lab_sample, lab_sample_sql, lab_autoeval          # noqa: E402

OUT = os.path.join(os.path.dirname(LAB), 'golden_set_example_rev1.xlsx')
...
rows = [
 ('구성', '수동 골든셋 1종 + 자동 평가셋 3종 + 정형 골든셋 + 스모크 20문항'),
 ...
 ('작성 순서', '① nb08 자동 생성 → ② 실패 문항 검토 → ③ 실제 업무 질문 추가 → 수동 골든셋 v1 확정'),
