...
OUT = 'golden_eval_pack_rev3.xlsx'
...
ws['A1'] = '골든셋 · 정량 평가 팩 rev.3'; ws['A1'].font = f(size=14, bold=True, color=PRI)
ws['A2'] = '2026-09-23 · 원본 확보 → 파싱 · 청킹 → 근거 연결 → 골든셋 확정 → 정량 평가 · 연노랑 = 기입 / 회색 = 자동 계산'
ws['A2'].font = f(size=9, color='6B6B6B')
steps = [
 ('0단계 · 선행 조건', '질문의 근거 문서가 사내 코퍼스에 적재 · 파싱 · 청킹된 상태여야 측정 성립', '미적재 문항 = 수집 과제 (검색 점수 제외)'),
...
wb._sheets = [wb[n] for n in ['0_안내', '1_골든셋_검수', '2_골든셋_확정', '2b_질의응답_골든셋', '2c_답변_채점기준', '2d_근거문서_연결', '2e_원본_수집목록',
